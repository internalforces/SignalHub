...
윈도우 CLI 분석을 포함한 CSV to Signal `0.3.0`은 npm에 `csv-to-signal`로 공개됐습니다.
아래 GitHub와 CoinGecko 명령은 저장소 빌드에 포함되며, 이미 공개된 npm
`csv-to-signal@0.3.0` 아티팩트에는 포함되지 않습니다. TASK-029는 새 npm 버전을 공개하지
않습니다. 스케줄링, 알림, REST API,
