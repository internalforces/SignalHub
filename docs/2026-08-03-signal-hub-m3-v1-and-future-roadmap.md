...
| TASK-M3-1 | Approve M3 design, dependency, config, and CLI contract | S | M3-0, M3-0b | Approvals record the YAML dependency, 1 MiB/no-alias and duplicate-key rejection, dedicated M3 database path, canonical identity digest and persistent metric namespace, dataset-selecting-header identity, horizon-bounded analysis with predecessor context and no horizon increases, exact grouped JSON contract including usage/config/selection/first-source failures, per-source atomicity, partial-failure contract, per-hop deadlines, and confirm no schema change. |
| TASK-M3-2 | Build config package | M | M3-1 | Pre-parse size/alias limits, duplicate-mapping-key rejection, header-only interpolation, schema validation including nonempty sources and horizon-increase rejection, duplicate-source-ID rejection, output-visible placeholder rejection, canonical identity digest derivation including declared dataset-selecting headers, and redaction tests pass. |
...
- Verify an empty sources list and a history-horizon increase for an established source fail as `config_invalid` before any request or persistence.
