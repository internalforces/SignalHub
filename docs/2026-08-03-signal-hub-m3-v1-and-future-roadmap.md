...
M3 keeps the shared DataPoint, Signal, Detector, and Connector contracts unchanged. Connectors normalize timestamps to ISO 8601 UTC before returning data. The existing `analyze` command continues to use `data.db`; the M3 `run` command uses the separate, fixed local `signal-hub-m3.db` file in the command's current working directory, with the existing SQLite schema unchanged. CSV input therefore never shares storage with M3 and cannot forge an M3 namespace. Inside that M3-only database, a configured `metricId` is the public identity only: after config validation derives `source.identityDigest`, the CLI constructs the opaque storage metric ID `m3:${encodeURIComponent(source.id)}:${source.identityDigest}` before the source enters Core or SQLite. `identityDigest` is the lowercase SHA-256 hex digest of the source's canonical, nonsecret provider-identity value, defined below. This namespaces a source's persisted points from other M3 sources and from a later configuration that retargets the same source ID or display metric ID, while a display-metric rename retains the same history. The CLI maps that private value back to the configured `metricId` only in the `run` JSON view; it never treats an existing un-namespaced series as M3 history. Before calling Core, every connector must emit at most one point for each storage metric ID/timestamp pair under the source-specific duplicate policy defined below. M3-1 must separately approve the narrow Core/Storage additions and finite-signal validation defined in this document; no implementation changes them before that approval.
...
The config package derives `identityDigest`; it is not a YAML field. It hashes UTF-8 canonical JSON with recursively lexicographically sorted object keys and no insignificant whitespace. The identity value includes `type`, `historyDays`, and every nonsecret source-specific field that selects or transforms provider data, but excludes `id`, `metricId`, all request headers, and every expanded environment value. A display rename therefore retains history, while a horizon or provider-identity change creates an independent history without making a secret-derived verifier available in SQLite or public output.

Every REST source requires a nonempty, stable, explicitly nonsecret `datasetId` literal. It identifies the logical dataset selected behind request headers: when a header-value change selects a different tenant, account, feed, or other dataset, the operator must change `datasetId`; a credential-only rotation retains it. `datasetId` cannot contain an environment placeholder or be derived from a header. REST URLs are also canonical nonsecret identity input and must have empty username, password, query, and fragment components; M3 defers generic query-parameter support, and all credentials must be supplied through request headers. Request-header names remain case-insensitively unique before interpolation or construction of Node's `Headers`, but header names and values never enter `identityDigest`. For REST, the exact identity value is `{ type, datasetId, url, recordsPointer, timestampPointer, valuePointer, bucket, aggregate, historyDays }`; changing any field creates an independent persistence namespace and public signal identity. Configuration mappings are closed, so any unknown key at any depth is `config_invalid`. TASK-M3-0b and TASK-M3-0 must respectively record complete, nonsecret CoinGecko and Polymarket identity-field lists before M3-1 approval. A source `id` remains stable for its intended logical source; changing it intentionally creates an independent history.
...
After a source first persists successfully, M3 accepts only append-only new timestamps within the normalized fetched output. Let `comparisonEnd` be the earlier of `end` and the source namespace's latest persisted timestamp. For every fetched point in `[start, comparisonEnd]`, Core requires an existing row at that timestamp: an equal value is an idempotent no-op, a different value is `historical_conflict`, and no existing row is `late_backfill`. This is not a completeness requirement on the provider response; persisted rows omitted from a later fetch, including rows before `start` and the optional detector-context predecessor, do not fail the source. Rejecting only newly discovered older observations avoids retaining stale signals whose percentage-change adjacency or threshold crossings would change. The source transaction rolls back fully on either failure.
...
Core treats detector output as untrusted numeric data. Before scoring it requires every raw signal's `value` and `changePercent` to be finite, and after scoring it also requires `score` to be finite. A detector exception or any non-finite result fails the source atomically with the redacted `pipeline_failed` code; no signal from that source is persisted or returned. This validation applies with or without `analysisWindow`, preserving the M1/M2 output shape for valid inputs while preventing JSON numeric fields from becoming `null`. It does not change detector formulas or scoring.

`SqliteStorage.transaction` runs `operation` synchronously inside one outer `better-sqlite3` transaction and returns its value. After connector fetch completes, Core uses this outer transaction for every pipeline invocation, with or without `analysisWindow`, and performs existing-value and late-backfill checks, point insertion, reads, detector execution, finite-signal validation, scoring/filtering, and signal insertion inside the callback. Any exception from those steps rolls back both points and signals. A callback that returns a Promise or other thenable is rejected from inside the transaction before commit; no asynchronous I/O is permitted in the callback. Existing repository methods participate in the outer transaction rather than committing independently, while direct repository calls made outside Core retain their M1/M2 behavior.
  ...
  "sources": [
    {
      ...
      "signals": [
        {
          "id": "[\"bitcoin-usd\",\"dbb5018b9f4195be73c64df1609dd9ccc6a13922ec6bf002bb2cdbbfaead8a22\",\"percentage-change\",\"crypto:bitcoin:usd\",\"2026-08-03T00:00:00.000Z\",100,21]",
          "metricId": "crypto:bitcoin:usd",
          "type": "increase",
          "score": 42,
          "direction": "up",
...
| --- | --- | --- | --- | --- |
| TASK-M3-0 | Define and approve the Polymarket public contract | S | — | The source schema, market/outcome selection, value meaning, timestamp/history semantics and horizon, normalized-timestamp collision and nonsecret identity-field policies, and human approval are recorded. |
| TASK-M3-0b | Define and approve CoinGecko history semantics | S | — | The endpoint, request parameters, horizon, observation-count expectation, timestamp semantics, provider limitations, normalized-timestamp collision and nonsecret identity-field policies, and human approval are recorded. |
| TASK-M3-1 | Approve M3 design, dependency, config, CLI, and narrow Core/Storage API contract | S | M3-0, M3-0b | Approvals record the YAML dependency, 1 MiB/no-alias, duplicate-key, closed-schema, case-insensitive-header, and REST URL component restrictions; dedicated M3 database path; canonical nonsecret identity digest, required REST `datasetId`, persistent metric namespace, and identity-scoped public signal IDs; one run-wide `analysisNow`; the exact additive `AnalysisWindow`/range-query and synchronous `SqliteStorage.transaction<T>` APIs; finite detector-output enforcement; horizon-bounded analysis with predecessor context; exact grouped JSON contract including usage/config/selection/first-source failures; per-source atomicity; partial-failure contract; per-hop deadlines; and confirm no database-schema change. |
| TASK-M3-2 | Build config package | M | M3-1 | Pre-parse size/alias limits, duplicate-mapping-key rejection, closed-schema and case-insensitive-header validation, header-only interpolation, required nonsecret REST `datasetId`, userinfo/query/fragment URL rejection, schema validation including nonempty sources and the maximum horizon, duplicate-source-ID rejection, output-visible placeholder rejection, canonical nonsecret identity-digest derivation, and redaction tests pass. |
...
| TASK-M3-6 | Add approved CLI composition and docs | M | M3-2 through M3-5 | Existing analyze tests stay green; implement only the approved Core range and Storage transaction APIs plus finite-signal validation; config-run tests cover one run-wide analysis time, namespaced persistence and identity changes, horizon-bounded reads with predecessor-only threshold context including empty fetches, historical conflicts, window-scoped late-backfill rejection, selection, order, source-atomic rollback, stable complete/partial/failed JSON, identity-scoped public ID projection, usage errors, and redaction. |
...
- Verify every REST source requires a literal nonsecret `datasetId`; placeholders and URLs containing userinfo, a query, or a fragment fail as `config_invalid`, and no expanded header value contributes to a persisted or public identifier.
...
- Verify a dataset-selecting REST header change accompanied by a new nonsecret `datasetId` derives a new namespace, while credential-only rotation with the same `datasetId` retains history; neither header value is hashed or persisted.
...
- Verify that two sources sharing a display metric ID and timestamp still publish distinct source-and-identity-scoped signal IDs.
...
- Verify a fetched point with no existing row in `[start, comparisonEnd]` fails with `late_backfill`, preserves prior points and signals, and emits no contradictory recomputed signals, while persisted timestamps omitted from the response or before the advancing `start` need not be refetched.
- Verify extreme finite inputs whose detector arithmetic produces a non-finite `changePercent` fail source-atomically as `pipeline_failed`; successful JSON never contains `null` in a numeric Signal field.
...
- Verify projected percentage-change and threshold signal IDs preserve their detector configuration, replace the private metric ID, and include both the source ID and nonsecret identity digest; retargeting the same source or changing its horizon cannot reuse a prior public signal ID.
