...
- **Release candidate**: `csv-to-signal@0.4.0` is an unmerged candidate being prepared from the reviewed M8 runtime modernization; it is not published
- **Phase**: TASK-030 release preparation is active; Node 22/24 verification, independent review, PR merge, exact-artifact approval, npm verification, and GitHub Release remain required
- **Next milestone**: Verify and independently review the `0.4.0` candidate without tagging or publishing
