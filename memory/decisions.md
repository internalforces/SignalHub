...
**Context**: The proposed M3 multi-source configuration and CLI contracts were incomplete around source identity, authorization interpolation, open buckets, failure behavior, provider history semantics, redirect safety, signal identity, and early-failure cases identified in PR #6 review.
**Decision**: Use an opaque per-source storage metric namespace while preserving configured metric IDs only in the public output; reject revised persisted values as `historical_conflict`; allow whole-value interpolation only in request headers; treat unbucketed duplicate timestamps as source failures; include a bucket that ends exactly at the injected current time; permit only manually validated same-origin redirects; project public signal IDs by replacing only their private metric tuple item; publish an exact grouped JSON contract for complete, partial, and all failed outcomes; make each source pipeline atomic; and make reviewed, human-approved Polymarket and CoinGecko contracts prerequisites to wider M3 approval.
**Rationale**: These rules prevent history from different commands or sources being combined, prevent stale or silently replaced values, protect credentials from cross-origin redirects, preserve signal identity, and ensure connector behavior and every JSON outcome are approved before implementation.
**Trade-offs**: M3 planning has two additional approval tasks, requires a storage-namespace/output projection layer and per-source transactions, rejects provider corrections pending a separately approved replacement policy, and intentionally uses explicit partial completion rather than a run-wide transaction.
